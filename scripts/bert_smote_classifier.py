#!/usr/bin/env python3
"""
BERT SMOTE Classifier - Simple but effective approach with SMOTE
...
Usage:
    python scripts/bert_smote_classifier.py --source reddit --mode train
"""
...
class SMOTEDataset(Dataset):
    """Dataset with SMOTE augmentation"""
    ...
    for epoch in range(epochs):
        ...
        if macro_f1 > best_val_f1:
            ...
            torch.save({
                ...
                'macro_f1': macro_f1,
                'micro_f1': micro_f1
            }, f'models/smote_{model_name}_best_{source}.pt')
...
def evaluate_model(model, test_loader, device, label_names, source, model_name=''):
    ...
    checkpoint_path = f'models/smote_{model_name}_best_{source}.pt'
...
def save_results(results, source, label_names, model_name=''):
    """Save results to output directory"""
    
    output_dir = f'output/{source}/bert_smote'
    ...
    with open(f'{output_dir}/bert_smote_{model_name}_metrics_{source}.json', 'w') as f:
    ...
    results_df.to_csv(f'{output_dir}/bert_smote_{model_name}_detailed_results_{source}.csv', index=False)
...
def main():
    parser = argparse.ArgumentParser(description='BERT SMOTE Classifier')
    ...
    if args.mode == 'train':
        ...
        if args.use_smote:
            texts, labels = apply_smote_simple(texts, labels)
        ...
        # Create datasets
        train_dataset = SMOTEDataset(train_texts, train_labels, tokenizer, args.max_length, augment=True)
        val_dataset = SMOTEDataset(val_texts, val_labels, tokenizer, args.max_length)
        test_dataset = SMOTEDataset(test_texts, test_labels, tokenizer, args.max_length)
        ...
        # Save results
        save_results(results, args.source, label_names, args.model)
