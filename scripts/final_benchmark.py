#!/usr/bin/env python3
"""
Final Benchmark: BERT, RoBERTa, Linear Regression, and SVM
...
Usage:
    python scripts/final_benchmark.py --source reddit --mode train
"""
...
def train_sklearn_models(X_train, y_train, X_test, y_test, label_names=None, source_name=None):
    ...
    lr_f1_macro = f1_score(y_test_binary, lr_pred, average='macro', zero_division=0)
    lr_f1_micro = f1_score(y_test_binary, lr_pred, average='micro', zero_division=0)
    ...
    svm_f1_macro = f1_score(y_test_binary, svm_pred, average='macro', zero_division=0)
    svm_f1_micro = f1_score(y_test_binary, svm_pred, average='micro', zero_division=0)
    ...
    rf_f1_macro = f1_score(y_test_binary, rf_pred, average='macro', zero_division=0)
    rf_f1_micro = f1_score(y_test_binary, rf_pred, average='micro', zero_division=0)
    ...
    # Build per-category summary rows for unified CSV if label_names provided
    if label_names is not None and source_name is not None:
        try:
            import pandas as pd
            import os
            ...
            rows = []
            # For traditional models, we don't have per-category precision/recall easily without probabilities.
            # We'll record per-category F1 where possible (sklearn_results don't include per-label), so we record global only and set per-category fields to NaN except category name.
            ...
            write_header = not os.path.exists(unified_csv_path)
            df.to_csv(unified_csv_path, mode='a', header=write_header, index=False)
            print(f"Appended traditional model rows to {unified_csv_path}")
            
            # Also append to overall summary CSV
            try:
                ...
                print(f"Appended traditional model overall summary to {overall_path}")
            except Exception as exc:
                print(f"Warning: failed to append traditional overall summary: {exc}")
        except Exception as exc:
            print(f"Warning: failed to append traditional results to unified CSV: {exc}")
...
def save_benchmark_results(all_results, source, label_names):
    ...
    # Save comparison
    comparison_df.to_csv(f'{output_dir}/benchmark_comparison.csv', index=False)
    ...
    for model_name, results in all_results.items():
        if 'label_f1_scores' in results:
            ...
            results_df.to_csv(f'{output_dir}/benchmark_{model_name}.csv', index=False)
...
def main():
    parser = argparse.ArgumentParser(description='Final Benchmark Classifiers')
    ...
    parser.add_argument('--skip_transformers', action='store_true', help='Skip transformer models (only run sklearn)')
    parser.add_argument('--sklearn_disable_smote', action='store_true', help='Do not use SMOTE for sklearn baselines')
    ...
    if args.mode == 'train':
        ...
        if args.sklearn_disable_smote:
            sklearn_texts, sklearn_labels = train_texts, train_labels
            print("Using raw training data for sklearn (SMOTE disabled)")
        else:
        ...
        sklearn_results = train_sklearn_models(X_train, sklearn_labels, X_test, test_labels, label_names=label_names, source_name=args.source)
