#!/usr/bin/env python3
"""
BERT Multiclass Classifier with Frozen Layers for Homelessness Content Analysis

This script provides a BERT-based multiclass classification system with frozen layers
for analyzing homelessness-related content across 16 categories. All BERT layers
are frozen except the final classification head.
...
Usage:
    python scripts/bert_frozen_layers_classifier.py --source reddit --dataset gold_subset --mode train
    python scripts/bert_frozen_layers_classifier.py --source reddit --dataset gold_subset --mode predict --input data.csv
    python scripts/bert_frozen_layers_classifier.py --source reddit --dataset gold_subset --mode evaluate
"""
...
def save_results(results, source, label_names):
    ...
    results_df.to_csv(f'{output_dir}/bert_frozen_classification_results_{source}.csv', index=False)
...
def main():
    parser = argparse.ArgumentParser(description='BERT Frozen Layers Classifier for Homelessness Content Analysis')
    ...
    if args.mode == 'train':
        ...
        # Load best model
        if os.path.exists(f'models/bert_frozen_best_{args.source}.pt'):
            model.load_state_dict(torch.load(f'models/bert_frozen_best_{args.source}.pt'))
        ...
        if not args.output:
            args.output = f'output/{args.source}/bert_frozen/predictions_{args.source}.csv'
        ...
        model_path = f'models/bert_frozen_best_{args.source}.pt'
