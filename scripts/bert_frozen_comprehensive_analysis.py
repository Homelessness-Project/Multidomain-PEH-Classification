#!/usr/bin/env python3
"""
Comprehensive BERT Frozen Layers Analysis Script

This script combines training, evaluation, and analysis into one comprehensive tool
that outputs macro and micro F1 scores for each category for each source using
frozen BERT layers (only the classification head is trainable).

Usage:
    python scripts/bert_frozen_comprehensive_analysis.py --mode train_evaluate
    python scripts/bert_frozen_comprehensive_analysis.py --mode evaluate_only
    python scripts/bert_frozen_comprehensive_analysis.py --mode predict_only --input data.csv
"""
...
def create_model_and_tokenizer(num_labels, model_name='bert-base-uncased'):
    """Create BERT model and tokenizer with frozen layers"""
    ...
    # Freeze all BERT layers except the final classification head
...
def train_model(model, train_loader, val_loader, device, epochs=5, learning_rate=2e-5, source=''):
    """Train the BERT model with frozen layers and early stopping"""
    ...
    print(f"Training for {epochs} epochs with frozen BERT layers...")
    ...
    for epoch in range(epochs):
        ...
        if best_macro_f1 > best_val_f1:
            ...
            torch.save(model.state_dict(), f'models/bert_frozen_best_{source}.pt')
...
def save_results(results, source, label_names):
    ...
    output_dir = f'output/{source}/bert_frozen'
    ...
    with open(f'{output_dir}/bert_frozen_metrics_{source}.json', 'w') as f:
    ...
    results_df.to_csv(f'{output_dir}/bert_frozen_detailed_results_{source}.csv', index=False)
...
def print_comprehensive_results(all_results):
    ...
    print("\n" + "="*100)
    print("COMPREHENSIVE BERT FROZEN LAYERS ANALYSIS RESULTS")
    print("="*100)
    ...
    # Save summary
    summary_df.to_csv('output/comprehensive_analysis/bert_frozen_summary_results.csv', index=False)
    
    # Save detailed results
    detailed_df.to_csv('output/comprehensive_analysis/bert_frozen_detailed_results.csv', index=False)
    
    print(f"\n" + "="*100)
    print("RESULTS SAVED TO:")
    print(f"- Summary: output/comprehensive_analysis/bert_frozen_summary_results.csv")
    print(f"- Detailed: output/comprehensive_analysis/bert_frozen_detailed_results.csv")
    print("="*100)
...
def create_performance_visualizations(all_results):
    ...
    plt.xlabel('Sources')
    plt.ylabel('F1 Score')
    plt.title('BERT Frozen Layers Performance by Source')
    plt.xticks(x, sources)
    plt.legend()
    plt.tight_layout()
    plt.savefig('output/comprehensive_analysis/plots/bert_frozen_overall_performance.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('BERT Frozen Layers F1 Scores by Category and Source')
    plt.tight_layout()
    plt.savefig('output/comprehensive_analysis/plots/bert_frozen_category_performance_heatmap.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def train_and_evaluate_single_source(source, args):
    ...
    print(f"\n{'='*80}")
    print(f"PROCESSING {source.upper()} WITH FROZEN BERT LAYERS")
    print(f"{'='*80}")
    ...
    if args.mode == 'train_evaluate':
        # Train model
        print("Training model with frozen BERT layers...")
        model = train_model(model, train_loader, val_loader, device, args.epochs, args.learning_rate, source)
        
        # Load best model
        if os.path.exists(f'models/bert_frozen_best_{source}.pt'):
            model.load_state_dict(torch.load(f'models/bert_frozen_best_{source}.pt'))
            print("Loaded best model for evaluation")
    
    elif args.mode == 'evaluate_only':
        # Load trained model
        model_path = f'models/bert_frozen_best_{source}.pt'
...
def main():
    parser = argparse.ArgumentParser(description='Comprehensive BERT Frozen Layers Analysis')
    ...
    print(f"BERT FROZEN LAYERS COMPREHENSIVE MULTICLASS ANALYSIS")
